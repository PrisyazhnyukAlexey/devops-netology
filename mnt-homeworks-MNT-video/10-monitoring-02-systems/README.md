...
```
Минимальный набор метрик: мониторинг  процессора, оперативной памяти, свободное место на диске и айноды
Так же если мы используем протокол http необходимо мониторить доступность этого ресурса  
```
#
...
inodes - это структура данных в которой хранится информация о файле или директории в файловой системе. В случаи если все inode уже используются , система заблокирует некоторые возможности, например, создание новых файлов.
CPUla - это загрузка процессора.
2.
В этом случаи можно внедоить систему  SLA, SLO и SLI.
...
```
В этом случаи выбираем из  opensource решений, такие как:
ELK( Elasticsearch, Logstash и Kibana), TIG(Telegraf, InfluxDB, Grafana)/TICK,Graylog, Fluentd, Sentry
...
```
ошибка в том, что не учли 300 код ответа, который говорит о том, что установлен редирект на другую страницу.
как вариант можно использовать следующую формулу
(summ_2xx_requests/(summ_2xx_requests + summ_4xx_requests + summ_5xx_requests)) * 100
```
#
...
![image](https://github.com/djohnii/devops-netology/assets/91311426/2ec2a169-ef25-45d7-91da-80c0ae8ed163)
...
![image](https://github.com/djohnii/devops-netology/assets/91311426/93fe83e7-7ce5-47f5-afff-6a1f34c687c7)
...
![image](https://github.com/djohnii/devops-netology/assets/91311426/8993c321-a6c0-4ae4-bdfa-a73b98f6171f)
